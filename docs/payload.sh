...
UNIT
  ...
  echo ""
  echo -e "  ${YELLOW}NGINX-Routen einrichten${NC} (falls noch nicht vorhanden):"
  echo -e "  Füge in die jam.$TLD Config diese zwei Locations ein:"
  echo ""
  echo -e "    ${BOLD}# Studio + Admin + Puck-Editor (URI unverändert)"
  echo -e "    location ~ ^/studio(/|\$) {"
  echo -e "        proxy_pass http://127.0.0.1:${PORT};"
  echo -e "        include /etc/nginx/proxy/payload.conf;"
  echo -e "    }"
  echo -e ""
  echo -e "    # Next.js-Assets + API + Media (basePath-Kompensat für Plugin-Hardcodes)"
  echo -e "    location ~ ^/(_next|api|next|media|__nextjs)(/|\$) {"
  echo -e "        rewrite ^(.*)\$ /studio\$1 break;"
  echo -e "        proxy_pass http://127.0.0.1:${PORT};"
  echo -e "        include /etc/nginx/proxy/payload.conf;"
  echo -e "    }${NC}"
  echo ""
  echo -e "  ${YELLOW}Wichtig:${NC} Der ${BOLD}\$${NC}-Anker in ^/studio(/|\$) ist kritisch — er fängt"
  echo -e "  ${BOLD}/studio${NC} ohne trailing slash ab, das sonst in try_files fallen und einen"
  echo -e "  301-Redirect-Loop mit Next.js' 308 erzeugen würde (Session 24/25 Fix)."
  echo ""
